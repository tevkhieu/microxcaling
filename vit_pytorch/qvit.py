...
class FeedForward(Module):
    def __init__(self, dim, hidden_dim, dropout = 0., mx_specs=None):
        super().__init__()
        self.norm = nn.LayerNorm(dim)

        self.net = nn.Sequential(
            mx.Linear(dim, hidden_dim, mx_specs),
            mx.GELU(),
            nn.Dropout(dropout),
            mx.Linear(hidden_dim, dim, mx_specs),
            nn.Dropout(dropout)
        )
...
class Attention(Module):
    def __init__(self, dim, heads = 8, dim_head = 64, dropout = 0., mx_specs=None):
        super().__init__()
        ...
        self.to_qkv = mx.Linear(dim, inner_dim * 3, bias=False, mx_specs=mx_specs)

        self.to_out = nn.Sequential(
            mx.Linear(inner_dim, dim, mx_specs=mx_specs),
            nn.Dropout(dropout)
...
class Transformer(Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, dropout = 0., mx_specs=None):
        super().__init__()
        self.norm = nn.LayerNorm(dim)
        ...
        for _ in range(depth):
            self.layers.append(ModuleList([
                Attention(dim, heads = heads, dim_head = dim_head, dropout = dropout, mx_specs=mx_specs),
                FeedForward(dim, mlp_dim, dropout = dropout, mx_specs=mx_specs)
            ]))
...
class QViT(Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, pool = 'cls', channels = 3, dim_head = 64, dropout = 0., emb_dropout = 0., decorr_sample_frac = 1., mx_specs=None):
        super().__init__()
        ...
        self.to_patch_embedding = nn.Sequential(
            ...
            mx.Linear(patch_dim, dim, mx_specs),
            nn.LayerNorm(dim),
        )
        ...
        self.transformer = Transformer(dim, depth, heads, dim_head, mlp_dim, dropout, mx_specs)

        self.pool = pool
        self.to_latent = nn.Identity()

        self.mlp_head = mx.Linear(dim, num_classes, mx_specs)
