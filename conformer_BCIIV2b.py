...
# Default to GPU 0; allow override with CUDA_VISIBLE_DEVICES from the shell.
if "CUDA_VISIBLE_DEVICES" not in os.environ:
    os.environ["CUDA_VISIBLE_DEVICES"] = "0"
gpus = [int(x) for x in os.environ["CUDA_VISIBLE_DEVICES"].split(",") if x.strip() != ""]
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x: Tensor, mask: Tensor = None) -> Tensor:
        ...
        # Use matmul form so MX op injection can quantize this path.
        energy = torch.matmul(queries, keys.transpose(-2, -1))  # batch, num_heads, query_len, key_len
        if mask is not None:
...
class ExGAN():
    def __init__(self, nsub):
        ...
        self.start_epoch = 0
        self.root = 'data/BCICIV_2b_mat/'

        self.pretrain = False

        os.makedirs("result", exist_ok=True)
        self.log_write = open("result/experiment.txt", "a")
        self.log_write.write("=== Subject %d ===\n" % self.nSub)
        ...
        for e in range(self.n_epochs):
            ...
            if (e + 1) % 1 == 0:
                ...
                wandb.log({
                    ...
                    "test_loss": float(loss_test.detach().cpu().numpy()),
...
def main():
    ...
    aver = 0
    os.makedirs("result", exist_ok=True)
    result_write = open("result/experiment_summary.txt", "w")
    ...
    result_write.close()
    wandb.log({
        "average_best_accuracy": float(best),
        "average_accuracy": float(aver),
    })
    wandb.finish()
